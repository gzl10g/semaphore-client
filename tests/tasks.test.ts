...
// — estados que nunca terminan solos (2.19.8) —

test("waitForCompletion corta si la aprobación fue rechazada", async () => {
  const resource = makeResource();
  mock.method(resource, "get", async () => makeTask({ status: "rejected" }));

  await assert.rejects(
    () => resource.waitForCompletion(1, 1),
    (e: unknown) => e instanceof SemaphoreApiError && /rejected/.test(String((e as Error).message)),
  );
});

test("waitForCompletion puede cortar en waiting_confirmation en vez de esperar a un humano", async () => {
  const resource = makeResource();
  mock.method(resource, "get", async () => makeTask({ status: "waiting_confirmation" }));

  await assert.rejects(
    () => resource.waitForCompletion(1, 1, { onWaitingConfirmation: "throw" }),
    (e: unknown) => e instanceof SemaphoreApiError && /approval/.test(String((e as Error).message)),
  );
});

test("waitForCompletion sigue esperando en stopping hasta stopped", async () => {
  const tm = mock.method(globalThis, "setTimeout", (fn: TimerHandler) => {
    if (typeof fn === "function") fn();
    return 0 as unknown as ReturnType<typeof setTimeout>;
  });
  ...
  const resource = makeResource();
  const statuses = ["starting", "running", "stopping", "stopped"] as const;
  let call = 0;
  mock.method(resource, "get", async () => makeTask({ status: statuses[Math.min(call++, 3)] }));

  const result = await resource.waitForCompletion(1, 1);
  assert.equal(result.status, "stopped");
  assert.equal(call, 4, "no debe cortar antes de llegar a un estado final");

  tm.mock.restore();
});

test("waitForCompletion por defecto espera la aprobación: waiting_confirmation → confirmed → success", async () => {
  const tm = mock.method(globalThis, "setTimeout", (fn: TimerHandler) => {
    if (typeof fn === "function") fn();
    return 0 as unknown as ReturnType<typeof setTimeout>;
  });
  ...
  const resource = makeResource();
  const statuses = ["waiting_confirmation", "waiting_confirmation", "confirmed", "running", "success"] as const;
  let call = 0;
  mock.method(resource, "get", async () => makeTask({ status: statuses[Math.min(call++, 4)] }));

  const result = await resource.waitForCompletion(1, 1);
  assert.equal(result.status, "success");
  assert.equal(call, 5, "confirmed no es final: la playbook todavía tiene que correr");
