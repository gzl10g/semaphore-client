...
export async function handleRepositoriesUpdate(
  ...
  reportMutation(opts, { message: `Repository ${id} updated`, id });
}
...
export async function handleRepositoriesBranches(
  projectFlag: number | undefined,
  id: number,
  opts: { json?: boolean },
  deps?: HandlerDeps,
  ...
  const client = resolveClient(deps);
  formatOutput(await client.repositories.branches(getProjectId(projectFlag, deps), id), opts);
}

export async function handleRepositoriesPlaybooks(
  projectFlag: number | undefined,
  id: number,
  opts: { json?: boolean },
  deps?: HandlerDeps,
  ...
  const client = resolveClient(deps);
  formatOutput(await client.repositories.playbooks(getProjectId(projectFlag, deps), id), opts);
}
