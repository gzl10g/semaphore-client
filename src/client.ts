...
import { GlobalRolesResource } from "./resources/roles.js";
import { InstanceTasksResource } from "./resources/instance-tasks.js";
import { BackupResource } from "./resources/backup.js";
...
export class SemaphoreClient {
  ...
  readonly roles: GlobalRolesResource;
  readonly instanceTasks: InstanceTasksResource;
  ...
  constructor(config: SemaphoreClientConfig) {
    ...
    this.workflows = new WorkflowsResource(request);
    this.integrations = new IntegrationsResource(request);
    this.roles = new GlobalRolesResource(request);
    this.instanceTasks = new InstanceTasksResource(request);
    this.backup = new BackupResource(request);
  }
  ...
  private async request<T = unknown>(endpoint: string, options?: RequestOptions): Promise<T> {
    ...
    let lastError = new SemaphoreApiError(0, "No attempts made", undefined, undefined, method, endpoint);
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      if (res.ok || res.status === 204) {
        ...
        // Not every 2xx carries JSON: the raw task output is text/plain, and
        // several handlers answer 200 with no body at all (validating a cron,
        // for one). Parsing those as JSON threw where nothing had failed.
        ...
        try {
          ...
          throw new SemaphoreApiError(
            res.status,
            `${res.statusText}, but the body is not valid JSON`,
            undefined,
      ...
      lastError = new SemaphoreApiError(res.status, res.statusText, undefined, body || undefined, method, endpoint);
